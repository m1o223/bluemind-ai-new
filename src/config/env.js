...
const envSchema = z.object({
  ...
  PASSWORD_RESET_SESSION_TTL_MINUTES: z.coerce.number().int().min(5).max(60).default(10),
  PASSWORD_RESET_MAX_RESEND_ATTEMPTS: z.coerce.number().int().min(1).max(10).default(3),
  PASSWORD_RESET_RESEND_LOCK_MINUTES: z.coerce.number().int().min(10).max(1440).default(60),
  ...
  OPENAI_TRANSCRIPTION_MODEL: z.string().trim().default("whisper-1"),
  OPENAI_TTS_MODEL: z.string().trim().default("tts-1"),
  OPENAI_TTS_VOICE: z.string().trim().default("alloy"),
  VOICE_AUDIO_MAX_BYTES: z.coerce.number().int().positive().default(12 * 1024 * 1024),
